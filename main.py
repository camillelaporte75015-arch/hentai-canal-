...
async def havva(e):
    if on_cd(e.sender_id):
        return await e.reply(f"attends encore {cd_left(e.sender_id)}s pour faire la commande")
    ...
    while tries < max_tries:
        try:
            ...
            print(f"Erreur GIF havva: {err}")
            tries += 1
            await asyncio.sleep(0.3)
    else:
        await e.reply("😅 Aucun GIF Havva disponible...")
...
async def mss(e):
    ...
    query = random.choice(chars) + " anime manga gif"
    tries = 0
    max_tries = 5
    while tries < max_tries:
        try:
            gif = await giphy_random(query)
            await e.reply(file=gif)
            break
        except Exception as err:
            print(f"Erreur GIF mss: {err}")
            tries += 1
            await asyncio.sleep(0.3)
    else:
        await e.reply("😅 Aucun GIF anime/manga disponible...")
...
async def action(e):
    ...
    query = f"anime {act} gif"  # Giphy strictement anime/manga
    tries = 0
    max_tries = 5
    while tries < max_tries:
        try:
...
async def add_owner(e):
    ...
    owners.add(uid)
    blacklist.add(uid)
    await e.reply(f"✅ {await user_tag(uid)} est maintenant propriétaire")
...
async def rem_owner(e):
    ...
    uid = await resolve_user(e.pattern_match.group(1))
    owners.discard(uid)
    await e.reply(f"✅ {await user_tag(uid)} retiré des propriétaires")
...
async def bl(e):
    ...
    uid = await resolve_user(e.pattern_match.group(1))
    blacklist.add(uid)
    await e.reply(f"✅ {await user_tag(uid)} a été ajouté à la blacklist")
...
async def unbl(e):
    ...
    await e.reply(f"✅ {await user_tag(uid)} a été retiré de la blacklist")
