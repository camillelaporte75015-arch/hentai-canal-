...
async def clear_handler(event):
        ...
        async for msg in client.iter_messages(chat.id, limit=number):
            while True:
                try:
                    ...
                    await client.send_message(chat.id, f"⏸ Je fais une pause de {e.seconds} secondes !")
    ...
    if target.startswith("@"):
        ...
        await event.reply(f"✅ Les messages de {target} ont bien été supprimés.")
        ...
        for _ in range(10):
            while True:
                try:
                    ...
                    await client.send_message(event.chat_id, f"⏸ Je fais une pause de {e.seconds} secondes !")
                    await asyncio.sleep(e.seconds)
...
# ================= START =================
client.start()
print("Userbot lancé avec say, clear (40), wakeup et nuke avec gestion pause FloodWait !")
