...
BOT_TOKEN = "8640125594:AAG9sZVyTCt3J9-sK2aEC8yLR5Az_YDBSMk"
OWNER_ID = 7891919458  # seul cet ID peut gérer la BL
GIPHY_API_KEY = "YLLksuIyKHZcaMKuAOYR1s27dz2uy8Xr"

# --- STRUCTURES ---
blacklist = {OWNER_ID}  # Seuls les utilisateurs ici peuvent utiliser les commandes
cooldowns = {}
...
def is_on_cooldown(user_id):
    return user_id in cooldowns and time.time() - cooldowns[user_id] < COOLDOWN

def cooldown_remaining(user_id):
    if user_id not in cooldowns:
        return 0
    return max(0, int(COOLDOWN - (time.time() - cooldowns[user_id])))
...
async def cmd_hentai(event):
    ...
    if is_on_cooldown(user_id):
        await event.reply(f"😡 Calme toi avec le bot attends {cooldown_remaining(user_id)}s")
        return

    update_cooldown(user_id)
    gif = random.choice(hentai_gifs)
    msg = await event.respond(gif)
    await asyncio.sleep(7)
    await msg.delete()

# .kiss et .slap
@client.on(events.NewMessage(pattern=r'\.(kiss|slap) @(\w+)'))
async def cmd_action(event):
    user_id = event.sender_id
    if not is_allowed(user_id):
        await event.reply("❌ Tu n'as pas le droit d'utiliser cette commande !")
        return
    ...
    initiator = event.sender.username or f"user{user_id}"
    emoji = "💋" if action_type == "kiss" else "👋"
    if action_type == "kiss":
        gif = get_giphy("anime kiss")
    else:
        gif = get_giphy("anime slap")
    await event.respond(f"@{initiator} {action_type} @{target_username} {emoji}\n{gif}")

# .bl pour ajouter à la BL
@client.on(events.NewMessage(pattern=r'\.bl (@?\w+|\d+)'))
async def cmd_bl(event):
    ...
    await event.reply(f"✅ {target} ajouté à la BL.")

# .unbl pour retirer
@client.on(events.NewMessage(pattern=r'\.unbl (@?\w+|\d+)'))
async def cmd_unbl(event):
    if event.sender_id != OWNER_ID:
        await event.reply("❌ Tu n'as pas le droit d'utiliser cette commande !")
        return
    target = event.pattern_match.group(1).lstrip("@")
    ...
    await event.reply(f"✅ {target} retiré de la BL.")

# .blacklist pour afficher
...
async def cmd_blacklist(event):
    if event.sender_id != OWNER_ID:
        ...
        return
    if not blacklist:
        await event.reply("✅ La BL est vide.")
    else:
