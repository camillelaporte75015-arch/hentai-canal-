...
# --- LISTES & DICTS ---
blacklist = {OWNER_ID}  # seuls les users dans la blacklist peuvent utiliser .hentai
owners = {OWNER_ID}      # seul le propriétaire peut gérer la blacklist
cooldowns = {}
bl_added_by = {}         # pour savoir qui a ajouté chaque personne à la blacklist
...
def get_cooldown_remaining(user_id):
    if user_id not in cooldowns:
        return 0
    return max(0, int(COOLDOWN_TIME - (time.time() - cooldowns[user_id])))
...
# --- FONCTION GIPHY ---
async def get_giphy_gif(action):
...
async def hentai(event):
    ...
    if is_on_cooldown(user_id):
        await event.reply(f"😡 Calme-toi avec le bot, attends {get_cooldown_remaining(user_id)}s")
        return
...
async def action(event):
    ...
    gif = await get_giphy_gif(action_type)
    if not gif:
        await event.reply("❌ Aucun GIF trouvé, réessaie plus tard !")
        return

    emoji = "💋" if action_type == "kiss" else "👋"
    await event.reply(f"@{initiator_username} {action_type} @{target_username} {emoji}")
...
async def add_blacklist(event):
    ...
    bl_added_by[identifier] = event.sender.username or event.sender_id
    await event.reply(f"✅ {identifier} ajouté à la blacklist par {bl_added_by[identifier]}")
...
async def remove_blacklist(event):
    ...
    bl_added_by.pop(identifier, None)
...
async def show_blacklist(event):
    ...
    for user_id in blacklist:
        if user_id != OWNER_ID:
            added_by = bl_added_by.get(user_id, "inconnu")
            msg += f"@ / {user_id} (ajouté par {added_by})\n"
    await event.reply(msg)

# --- RUN CLIENT ---
async def main():
    await client.start()  # demandera ton numéro + code une fois
